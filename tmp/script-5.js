    ...
    // Global game state
        ...
        const riddlesData = {
            logic: {
                ...
                hard: [
                    ...
                    {
                        riddle: "A woman shoots her husband, then holds him under water for five minutes, then hangs him. Afterwards they both go out to dinner together. How?",
        ...
        (function enhanceAudioAndAuth() {
            ...
            const defaultTracks = [
                ...
                'assets/audio/Pokémon Theme Song.mp3',
    ...
    function drawFromPool(name, difficulty, allItems, count) {
        ...
        function handleRiddleCorrect() {
            ...
            showFeedback('riddles', 
                `🎉 Correct! The answer was "${riddles.currentRiddle.answer}". You earned ${totalScore} points! Click "New Riddle" to continue.`, 
                'success'
            );
        ...
        function handleRiddleIncorrect() {
            ...
            if (riddles.attemptCount >= riddles.maxAttempts) {
                ...
                showFeedback('riddles', 
                    `❌ Incorrect! The answer was "${riddles.currentRiddle.answer}". Click "New Riddle" to try another!`, 
                    'error'
                );
            } else {
                const attemptsLeft = riddles.maxAttempts - riddles.attemptCount;
                showFeedback('riddles', 
                    `❌ Incorrect! You have ${attemptsLeft} attempt${attemptsLeft !== 1 ? 's' : ''} left. Try again!`, 
                    'warning'
                );
            }
        ...
        function getRiddleHint() {
            ...
            if (riddles.currentHintLevel >= hints.length) {
                showFeedback('riddles', '💡 No more hints available for this riddle.', 'warning');
                return;
            }
            ...
            showFeedback('riddles', `💡 Hint ${hintNumber}: ${currentHint} (-${pointPenalty} points)`, 'info');
        ...
        function handleBurbleWin() {
            ...
            showFeedback('burble', `🎉 Correct! The word was "${burble.currentWord}"`, 'correct');
        ...
        function handleBurbleIncorrect(guess) {
            ...
            showFeedback('burble', `❌ "${guess}" is incorrect. ${feedback}`, 'incorrect');
        ...
        function getBurbleHint() {
            ...
            showFeedback('burble', `💡 Hint ${burble.hintsUsed}: ${hints[hintIndex]} (-25 points)`, 'hint');
        ...
        function handleEmojiWin() {
            ...
            showFeedback('emoji', `🎉 Correct! The answer was "${emoji.currentPuzzle.answer}"`, 'correct');
        ...
        function handleEmojiIncorrect() {
            showFeedback('emoji', '❌ Incorrect! Try again or get a hint.', 'incorrect');
            try { const user = JSON.parse(localStorage.getItem('burbleUser') || 'null'); if (user && user.username) recordIncorrect(user.username); } catch (e) {}
        }
        ...
        function getEmojiHint() {
            ...
            showFeedback('emoji', `💡 Hint: ${emoji.currentPuzzle.hint} (-5 points)`, 'hint');
            updateEmojiDisplay();
        }
        ...
        function makeValentineGuess() {
            ...
            if (guess.toLowerCase() === valentine.currentAnswer.toLowerCase()) {
                ...
                if (valentine.guessesLeft === 0) {
                    ...
                    showFeedback('valentine', `❌ Not quite! You have ${valentine.guessesLeft} guess(es) left.`, 'incorrect');
                    updateValentineDisplay();
                }
            }
        }
        ...
        function handleValentineWin() {
            ...
            showFeedback('valentine', `🎉 Correct! I was thinking of "${valentine.currentAnswer}"`, 'correct');
        ...
        function handleValentineLose() {
            ...
            showFeedback('valentine', `😔 The answer was "${valentine.currentAnswer}". Better luck next time!`, 'incorrect');
        ...
        function submitTriviaAnswer() {
            ...
            if (isCorrect) {
                ...
                showFeedback('trivia', '✅ Correct!', 'correct');
                try {
                    const user = JSON.parse(localStorage.getItem('burbleUser') || 'null');
                ...
                showFeedback('trivia', `❌ Incorrect. The correct answer was: ${question.options[question.correct]}`, 'incorrect');
                try { const user = JSON.parse(localStorage.getItem('burbleUser') || 'null'); if (user && user.username) recordIncorrect(user.username); } catch (e) {}
            }
        ...
        function endTriviaGame() {
            ...
            document.getElementById('trivia-game-over').innerHTML = `
                <div class="game-over ${won ? 'win' : 'lose'}">
                    <h3>${won ? '🎉 Great Job!' : '📚 Keep Studying!'}</h3>
        ...
        function submitAnimalTriviaAnswer() {
            ...
            if (isCorrect) {
                ...
                showFeedback('animal', '✅ Correct!', 'correct');
                try { const user = JSON.parse(localStorage.getItem('burbleUser') || 'null'); if (user && user.username) recordCorrect(user.username, 0); } catch (e) {}
            } else {
                showFeedback('animal', `❌ Incorrect. The correct answer was: ${question.options[question.correct]}`, 'incorrect');
                try { const user = JSON.parse(localStorage.getItem('burbleUser') || 'null'); if (user && user.username) recordIncorrect(user.username); } catch (e) {}
            }
        ...
        function endAnimalTriviaGame() {
            ...
            document.getElementById('animal-game-over').innerHTML = `
                <div class="game-over ${won ? 'win' : 'lose'}">
                    <h3>${won ? '🦁 Animal Expert!' : '🐾 Keep Learning!'}</h3>
        ...
        document.body.insertAdjacentHTML('beforeend', `
            ...
            <div id="settings-gear" class="settings-gear" title="Settings">⚙️</div>
        `);
    ...
    const signinBtn = document.getElementById('signin-btn');
        ...
        function renderLeaderboard() {
            ...
            rows.forEach((r, i) => {
                ...
                const avg = r.correctCount ? ((r.totalCorrectTime || 0) / r.correctCount).toFixed(1) : '—';
            ...
            "School Trivia": {
                Math: {
                    ...
                    Expert: [
                        ...
                        { q: "What is Taylor series expansion of e^x?", a: "1 + x + x²/2! + x³/3! + …" },
                        { q: "Define eigenvalue.", a: "λ such that Av = λv for some nonzero v." },
                ...
                Science: {
                    ...
                    Hard: [
                        { q: "What is Newton’s second law of motion?", a: "Force = mass × acceleration (F=ma)" },
                        { q: "What is the acceleration due to gravity on Earth (m/s²)?", a: "9.8" },
                        { q: "What is DNA’s shape called?", a: "Double helix" },
                    ...
                    Expert: [
                        ...
                        { q: "Define Heisenberg’s uncertainty principle.", a: "You cannot simultaneously know position and momentum exactly." },
                        { q: "What is Schrödinger’s equation used for?", a: "Describing quantum systems." },
                        { q: "What is CRISPR?", a: "A gene-editing technology." },
                        { q: "What is the Higgs boson?", a: "A particle giving mass via Higgs field." },
                        { q: "Define thermodynamics’ 2nd law.", a: "Entropy of an isolated system never decreases." },
                        { q: "What is a black hole singularity?", a: "A point of infinite density." },
                        { q: "What is the speed of light in vacuum (m/s)?", a: "3 × 10⁸ m/s" },
                        { q: "What is an isotope?", a: "Atoms of same element with different neutrons." },
                        { q: "What is plate tectonics?", a: "Theory Earth’s plates move causing earthquakes and volcanoes." }
                    ]
                },
